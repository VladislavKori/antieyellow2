...
import { createPhoto, editPhoto, getPhotos } from '../../../redux/actions/galaryActions';
...
function EditPhoto({photoid}: EditPhoto) {
    ...
    useEffect( () => {
        if (galary.photos) {
            setTitle(galary.photos[photoid].title)
        }
    }, [galary.photos])
    ...
    const editPhotoHandler = () => {
        if (file.current == null || title == "") return console.log('hello');
        form.append('title', title);
        form.append('img', file.current);
        form.append('photoid', galary.photos[photoid].id)
        dispatch(editPhoto({ data: form }))
    }
